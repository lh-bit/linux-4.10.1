...
bool osnet_enable_hyperviosr_helper_timer = false;
bool osnet_enable_hypervisor_hrtimer = false;
EXPORT_SYMBOL_GPL(osnet_kvm_in_lapic);
EXPORT_SYMBOL_GPL(osnet_kvm_x86_ops_in_lapic);
EXPORT_SYMBOL_GPL(osnet_hypervisor_core);
EXPORT_SYMBOL_GPL(osnet_enable_hyperviosr_helper_timer);
EXPORT_SYMBOL_GPL(osnet_enable_hypervisor_hrtimer);
#endif

#if OSNET_DTID_HRTIMER_EMULATE_TIMER
static ktime_t osnet_poll_pir(struct kvm_vcpu *vcpu, int vector)
{
    ktime_t delta = 0;
    struct kvm_x86_ops *ops = osnet_kvm_x86_ops_in_lapic;
    void *pir = (void *) ops->osnet_get_pir(vcpu);

    delta = ktime_get();
    while(test_bit(vector, pir));
    delta = ktime_sub(ktime_get(), delta);

    return delta;
}

static ktime_t osnet_set_target_expiration(struct hrtimer *timer,
                                           ktime_t periodicity)
{
    ktime_t prev_expiration = hrtimer_get_expires_tv64(timer);
    ktime_t next_expiration = prev_expiration + periodicity;
    return next_expiration;
}

static struct kvm_vcpu *osnet_get_vcpu(struct hrtimer *mytimer)
{
    struct osnet_vcpu_hrtimer *vcpu_hrtimer =
        container_of(mytimer, struct osnet_vcpu_hrtimer, timer);
    return vcpu_hrtimer->vcpu;
}

static enum hrtimer_restart osnet_callback(struct hrtimer *timer)
{
    ktime_t expiration = 0;
    ktime_t periodicity = ktime_set(0, NSEC_PER_SEC / OSNET_GUEST_HZ);
    struct kvm_vcpu *vcpu = osnet_get_vcpu(timer);
    struct kvm_x86_ops *ops = osnet_kvm_x86_ops_in_lapic;

    ops->osnet_set_pir_on(vcpu, OSNET_TIMER_INTERRUPT);
    expiration = osnet_set_target_expiration(timer, periodicity);
...
{
    ktime_t expiration = 0;
    ktime_t buffer = ktime_set(0, OSNET_BUFFER_NS);
    ktime_t periodicity = ktime_set(0, NSEC_PER_SEC / OSNET_GUEST_HZ);
    struct kvm_vcpu *vcpu = vcpu_timer->vcpu;
    struct kvm_x86_ops *ops = osnet_kvm_x86_ops_in_lapic;
    struct hrtimer *timer = &vcpu_timer->timer;

    hrtimer_init(timer, CLOCK_MONOTONIC, HRTIMER_MODE_ABS_PINNED);
    timer->function = &osnet_callback;

    ops->osnet_set_pir_on(vcpu, OSNET_TIMER_INTERRUPT);
    osnet_poll_pir(vcpu, OSNET_TIMER_INTERRUPT);
    
    expiration = ktime_get() + periodicity - buffer;
    hrtimer_start(timer, expiration, HRTIMER_MODE_ABS_PINNED);
    pr_info("%s\t%d\t%llu\n", __func__, vcpu->vcpu_id, ktime_get());
}
...
#if OSNET_DTID_LAPIC_TIMER_EMULATE_TIMER
...
#if OSNET_TRACE_DTID_LAPIC_TIMER_EMULATE_GUEST_TIMER 
    trace_printk("%s\t%llu\n", __func__, ktime_get());
#endif
}
#endif
...
#if OSNET_DTID_HRTIMER_EMULATE_TIMER
    if (cpu == osnet_hypervisor_core && osnet_enable_hypervisor_hrtimer)
        osnet_hrtimer_emulate_guest_timer();
#endif

#if OSNET_DTID_LAPIC_TIMER_EMULATE_TIMER
    if (cpu == osnet_hypervisor_core && osnet_enable_hyperviosr_helper_timer)
        osnet_emulate_guest_timer();
#endif
}
